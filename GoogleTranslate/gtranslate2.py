...
def translate_url(to_translate, to_language="auto", language="auto"):
    return "https://translate.google.com/m?hl=%s&sl=%s&q=%s"% (to_language, language, to_translate.replace(" ", "+"))
...
def transform(to_translate, to_language="auto", language="auto"):
    # send request
    r = requests.get(translate_url(to_translate, to_language, language))
...
for LANG in LANGS:
    ...
    # if you need to extract html tags
            ...
            if(totranslate!=None):
                arr[i]=translate_url(totranslate,LANG,INPUTLANGUAGE)
        if(root[i].tag=='string-array'):
            arr[i] = [None] * len(root[i])
            for j in range(len(root[i])):
    #	for each translatable string call the translation subroutine
    #   and replace the string by its translation,
                isTranslatable=root[i][j].get('translatable')
                if(arr[i][j].tag=='item') & (isTranslatable!='false'):
    # Here you might want to replace root[i].text by the findall_content function
    # if you need to extract html tags
                    # ~ totranslate="".join(findall_content(str(ET.tostring(root[i][j])),"item"))
                    totranslate=root[i][j].text
                    if(totranslate!=None):
                        arr[i][j]=translate_url(totranslate,LANG,INPUTLANGUAGE)
